...
STATIC_DATASET_PATH = r'../../bigcoal/103-102-part2'
...
def convert_bbox2labels(bbox):
    ...
    需要注意的是：该函数并没有写的很通用，他只写出了只有两个锚框的情形
    """
    ...
    for i in range(len(bbox)//5):
        ...
        labels[gridy, gridx, 0+5*i:5+5*i] = np.array([gridpx, gridpy, bbox[i * 5 + 3], bbox[i * 5 + 4], 1])
...
def create_csv_txt(img_dir, anno_dir, save_root_dir, train_val_ratio=1, padding=10, debug=False):
    """
    TODO:
    ...
    #     print("labels done.")
    save_img_dir = os.path.join(anno_dir, 'images')
    print(save_img_dir)
    if not os.path.exists(save_img_dir):
        os.mkdir(save_img_dir)
        # img_augument(img_dir, save_img_dir, labels_dir)
    imgs_list = os.listdir(save_img_dir) #列出图目录下所有的图片
    print(len(imgs_list))
    ...
    traincsv = np.zeros((n_train,5*3),dtype=np.float32)
...
if __name__ == '__main__':
    ...
    img_dir = os.path.join(STATIC_DATASET_PATH, "images")  # 原始图像文件夹
    anno_dirs = [STATIC_DATASET_PATH]  # 标注文件
    save_dir = os.path.join(STATIC_DATASET_PATH, "train")  # 保存处理后的数据(图片+标签)的文件夹
